...
v2.0-ML: Integrates AdaptiveBrain feedback loop:
- After each action, on_step() is called with reward signal
- At game_ended, on_game_end() triggers full Q-learning training pass
- ML stats logged and persisted via AgentMemory
"""
import json
import asyncio
import websockets
...
from bot.strategy.brain import decide_action, reset_game_state, learn_from_map, get_adaptive_brain
...
class WebSocketEngine:
    ...
    def __init__(self, game_id: str, agent_id: str):
        ...
        # ML tracking
        self._ml_prev_view = None       # View before last action
        self._ml_last_action_idx = None # Action index sent to ML
        self._ml_last_action_success = True
        self._ml_brain = get_adaptive_brain()
    ...
    async def _handle_message(self, msg: dict) -> dict | None:
        ...
        # Per actions.md: canAct and cooldownRemainingMs are at TOP LEVEL
            ...
            self._ml_last_action_success = success
        ...
        # ── game_ended ────────────────────────────────────────────────
            ...
            log.info("═══ GAME ENDED ═══")
            # ── ML: terminal training pass ────────────────────────────
            await self._ml_on_game_end(msg)
            reset_game_state()  # Clear per-game state
    ...
    async def _on_agent_view(self, view: dict):
        ...
        # ── ML: feedback from previous step ─────────────────────────
        # When we have a previous view + action, provide reward signal
        if (self._ml_prev_view is not None
                and self._ml_last_action_idx is not None):
            try:
                self._ml_brain.on_step(
                    self._ml_prev_view,
                    self._ml_last_action_idx,
                    self._ml_last_action_success,
                    view,
                )
            except Exception as ml_e:
                log.debug("ML on_step error (non-fatal): %s", ml_e)

        # Run strategy brain (ML-enhanced)
        ...
        ml_action = decision.get("ml_action", "unknown")
        ml_action_idx = decision.get("ml_action_idx", 15)  # default: wait
        ...
        log.info("→ %s | %s%s", action_type.upper(), reason,
                 f" [🧠ML:{ml_action}]" if not safety_override else " [🛡️SAFETY]")

        # ── Record state for next ML step ────────────────────────────
        self._ml_prev_view = view
        self._ml_last_action_idx = ml_action_idx
        self._ml_last_action_success = True  # Optimistic; updated by action_result

        # Feed dashboard with action (include ML indicator)
        ml_indicator = "🧠" if not safety_override else "🛡️"
        dashboard_state.update_agent(self.dashboard_key, {
            "last_action": f"{ml_indicator}{action_type}: {reason[:55]}",
            "ml_action": ml_action,
            "ml_stats": self._ml_brain.stats(),
        })
        dashboard_state.add_log(
            f"{ml_indicator}{action_type}: {reason[:75]}", "info", self.dashboard_key
        )

    async def _ml_on_game_end(self, game_msg: dict):
        """ML terminal training pass at game end."""
        try:
            ml_stats = self._ml_brain.on_game_end(game_msg, self.last_view)
            log.info(
                "🧠 ML game end: ε=%.4f, states=%d, reward=%.1f, games=%d",
                ml_stats.get("epsilon", 0),
                ml_stats.get("qtable_states", 0),
                ml_stats.get("game_reward", 0),
                ml_stats.get("train_games", 0),
            )
            # Tell dashboard ML stats
            dashboard_state.update_agent(self.dashboard_key, {
                "ml_stats": ml_stats,
                "last_action": (
                    f"🧠 TRAINING COMPLETE — ε={ml_stats.get('epsilon',0):.4f} "
                    f"states={ml_stats.get('qtable_states',0)} "
                    f"reward={ml_stats.get('game_reward',0):.0f}"
                ),
            })
            dashboard_state.add_log(
                f"🧠 ML trained: ε={ml_stats.get('epsilon',0):.4f} "
                f"states={ml_stats.get('qtable_states',0)}",
                "info", self.dashboard_key,
            )
            # Store ML stats in memory for persistence (heartbeat calls memory.save)
            # We store via dashboard state so heartbeat can pick it up
            dashboard_state.ml_stats = ml_stats
        except Exception as e:
            log.warning("ML on_game_end error (non-fatal): %s", e)
        finally:
            # Reset per-game ML tracking
            self._ml_prev_view = None
            self._ml_last_action_idx = None
